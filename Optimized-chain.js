...
words[3] = ["xbc", "pcxbcf", "xb", "cxbc", "pcxbc"];
words[4] = [
  "ksqvsyq",
  "ks",
  "kss",
...
function searchLonguestChain(predecessor, successor, chain, p) {
    ...
    if(count == lengthOfSucc){
        ...
        do {
            ...
            console.log(count, lengthOfSucc)
        ...
        return finalChain = finalChain.map(element => element.length ).reduce((max, element) => Math.max(max,element))
    }
}
...
for (let i = min; i < max ; i++) {
  ...
  for (let j = 0; j < lengthOfPredc; j++) {
    ...
    if (chain[i - 1] === undefined) {
      element = searchLonguestChain(predecessor[j], successor, chain, min + 1)
      longuestChain = Math.max(longuestChain , (typeof element == 'number' ? element : 1))
    } else {
      ...
      if (compteur == lengthOfPredChain) {
        element = searchLonguestChain(predecessor[j], successor, chain, i + 1)
        longuestChain = Math.max(longuestChain , (typeof element == 'number' ? element : 1))
      }
    }
  }
}
console.log(' The longuestChain is  ', longuestChain)
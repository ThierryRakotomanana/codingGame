...
words[2] = ["ksqvsyq","ks","kss","czvh","zczpzvdhx","zczpzvh","zczpzvhx","zcpzvh","zczvh","gr","grukmj","ksqvsq","gruj","kssq","ksqsq","grukkmj","grukj","zczpzfvdhx","gru"]
words[4] = ["a","bab"]
let word = ['texte','texte','texte','texte','texte','texte','texte','texte','texte','texte','texte','texte','texte',
...
function indexOfString(arrPrev, arr){
  ...
  function compareValue(value , arr, p, chain, longChain, initialPosition){
    let compteur = 0
      for(let i = 0; i < arr.length; i++ ){
          if(indexOfString(value , arr[i])){
              if(arr != chain[chain.length -1]){
                compareValue(arr[i] , chain[p+1], p+1, chain, longChain, initialPosition)
              }else{
                longChain.push(p + 1 - initialPosition)
              }
          }else {
            compteur++
          }
      ...
      if(compteur == arr.length){
        longChain.push(p - initialPosition)
    }
  }
...
function havePredecessor(firstArr, secondArr, longChain, p, chain){
    ...
    let del = ordinary(deleteDoublon(length(words[0])))
    let chain = new Map() , longChain = [], delLength = del.length, chainLength
    ...
    finalValue = ordinary(longChain)
    console.log(finalValue)
    finalValue[finalValue.length - 1]

    /**
     * return value : avoid to take multiple parameter
     * {@chain} : secondly, set a Map object, to make an order of succession by length
     * 
     * */